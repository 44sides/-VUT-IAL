...
int hC = hashCode(key); // hash code
tHTItem *new_el;

// zkusime najit prvek se stejnym klicem
tHTItem *tmp = htSearch(ptrht, key);

if(tmp != NULL)
    tmp->data = data;
else{
	new_el = malloc(sizeof(struct tHTItem));

	if(new_el == NULL) // kdyz se nepodarilo alokovat pamet'
		return;
		
    new_el->key = key;
    new_el->data = data;
    new_el->ptrnext = (*ptrht)[hC];

	(*ptrht)[hashCode(key)] = new_el;
}
}
...
if(tmp == NULL)
	...
	return &tmp->data; // ukazatel na polozku
}
...
tHTItem *tmp = (*ptrht)[hC]; // soucasny
if(tmp == NULL)
	return;
tHTItem *prevtmp = NULL; // predchozi
...
while(tmp != NULL)
{
    if(strcmp(key, tmp->key)){
		...
		// je na zacatku
        if(prevtmp == NULL)
			(*ptrht)[hC] = tmp->ptrnext; // nastaveni nasledujiciho prvku
        else // predchozi neni NULL
            prevtmp->ptrnext = tmp->ptrnext; // vynechani tmp
