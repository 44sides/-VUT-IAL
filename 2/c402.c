...
while(!SEmptyP(&stack)){
	RootPtr = STopPopP(&stack); // odber vrcholu stacku
...
bool RootStatus; // stav Root. true - poprve, false - podruhe
...
while(!SEmptyP(&stackP)){
	...
	RootStatus = STopPopB(&stackB);

	if (RootStatus == true){ // kdyz je poprve
		SPushP(&stackP, RootPtr);
		RootStatus = false;
		SPushB(&stackB, RootStatus);
...
{
	// prejdeme na pravy podstrom
	if (*RootPtr == NULL)
	{
		*RootPtr = STopPopP(&stack);
	}

	// ulozeni pravych podstromu behem pruchodu
	if ((*RootPtr)->RPtr)
	{
		SPushP(&stack, (*RootPtr)->RPtr);
	}
	
	// uvolneni a prechod dal na levy podstrom
	tmp = *RootPtr;
	*RootPtr = (*RootPtr)->LPtr;
	free(tmp);
}
