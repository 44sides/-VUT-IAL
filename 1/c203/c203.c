...
** a b_quindex. Všechny implementované funkce musí předpokládat velikost pole
...
if(q == NULL) queueError(QERR_INIT); // podminka
else{
	for(int i = 0; i < QUEUE_SIZE; i++){ // zaplnujeme celou frontu hvezdicky
		q->arr[i] = '*';
	}
	q->f_index = 0; // index prvniho pismena
	q->b_index = 0; // index nasledujici volne bunky stejny jako f_index
}
}
...
return (index + 1) % QUEUE_SIZE; // operace modulo => na indexu QUEUE_SIZE se uskutecni prechod na index 0
}
...
return (q->f_index == q->b_index) ? 1 : 0; // Ternary operator. Fronta prazdna pokud se indexy rovnaji
}
...
return (nextIndex(q->b_index) == q->f_index) ? 1 : 0; // kdyz se nechala jedna volna bunka, tak fronta je plna
}
...
if(queueEmpty(q) != 0) queueError(QERR_FRONT); // vsechny podminky
else *c = q->arr[q->f_index]; // vraceni pomoci pointeru
}
...
if(queueEmpty(q) != 0) queueError(QERR_REMOVE); // vsechny podminky
else q->f_index = nextIndex(q->f_index); // zacatek cteni se meni na jednou pozici
...
if(queueEmpty(q) != 0) queueError(QERR_GET); // podminky
else{
	queueFront(q, c); // vrati znak
	queueRemove(q); // odstrani znak
}
}
...
if(queueFull(q) != 0) queueError(QERR_UP); // podminky
else{
	q->arr[q->b_index] = c; // vlozeni
	q->b_index = nextIndex(q->b_index); // posun nasledujici volne bunky
}
}
